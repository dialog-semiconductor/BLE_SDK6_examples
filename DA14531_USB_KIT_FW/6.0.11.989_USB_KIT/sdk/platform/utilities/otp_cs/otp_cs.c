...
static bool xtal32m_trim_present = false;
...
void otp_cs_load_pd_rad(void)
{
    for (int i = 0; i < otp_cs.pd_rad_pairs; i++)
    {
        ...
        if ((otp_cs.pd_rad[i].addr == RF_PA_CTRL_REG) &&
            ...
            (rf_pa_pwr_get() > RF_TX_PWR_LVL_PLUS_0d6))         // 0dBm mode for all power setting 9 and below - 3dBm for 10 and above
            {
