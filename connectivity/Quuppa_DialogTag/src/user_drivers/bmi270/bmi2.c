 ...
 * @retval BMI2_E_COM_FAIL - Error: Communication fail
 */
static int8_t upload_file(uint32_t config_data_addr, uint16_t index, uint16_t write_len, struct bmi2_dev *dev);
 ...
 * @retval BMI2_E_INVALID_PAGE - Error: Invalid Page
 */
int8_t get_feat_config(uint8_t sw_page, uint8_t *feat_config, struct bmi2_dev *dev);
...
int8_t bmi2_sec_init(struct bmi2_dev *dev)
{
    ...
    /* Null-pointer check */
    rslt = BMI2_OK; // rslt = null_ptr_check(dev);
    if (rslt == BMI2_OK)
    {
...
int8_t bmi2_get_regs(uint8_t reg_addr, uint8_t *data, uint16_t len, const struct bmi2_dev *dev)
{
    ...
    if ((rslt == BMI2_OK) && (data != NULL))
    {
        ...
        if (dev->intf == BMI2_SPI_INTERFACE || dev->intf == BMI2_SPI_3W_INTERFACE)
        {
...
int8_t bmi2_soft_reset(struct bmi2_dev *dev)
{
    ...
    if (rslt == BMI2_OK)
    {
         ...
         */
        if ((rslt == BMI2_OK) && ((dev->intf == BMI2_SPI_INTERFACE) || (dev->intf == BMI2_SPI_3W_INTERFACE)))
        {
            rslt = bmi2_get_regs(BMI2_CHIP_ID_ADDR, &dummy_read, 1, dev);
            if (dev->intf == BMI2_SPI_3W_INTERFACE) {
                dummy_read = 0x01;
                bmi2_set_regs(BMI2_IF_CONF_ADDR, &dummy_read, 1, dev);
            }
        }
...
{
    uint16_t temp;
	  uint8_t temp_config_data_array[2] = { 0 };
		uint32_t actual_size;

    port_bmi270_itf_end();
		flash_itf_init();
		spi_flash_release_from_power_down();    
    spi_flash_read_data(temp_config_data_array, dev->config_file_addr+BMI2_CONFIG_INFO_LOWER, 2, &actual_size);    
		spi_flash_power_down();
		flash_itf_de_init();
    port_bmi270_itf_begin();
		
	/* extract the config  information */
    temp = (uint16_t)((uint16_t)(temp_config_data_array[0]) | ((uint16_t)(temp_config_data_array[1]) << 8));
    *config_major = BMI2_GET_BITS(temp, BMI2_CONFIG_MAJOR);
    *config_minor = BMI2_GET_BIT_POS0(temp_config_data_array[0], BMI2_CONFIG_MINOR);
    ...
    if (rslt == BMI2_OK)
    {
  			...
  			if (rslt == BMI2_OK)
        {
            if (!remain)
            {
                ...
                {
                    rslt = upload_file((dev->config_file_addr + index), index, dev->read_write_len, dev);
                }
            }
            else
            {
...
static int8_t upload_file(uint32_t config_data_addr, uint16_t index, uint16_t write_len, struct bmi2_dev *dev)
{
    ...
    uint8_t addr_array[2] = { 0 };
		...
		uint8_t* config_data = (uint8_t*) config_data_addr;
    if (config_data != NULL)
    {
...
//    int8_t rslt;
//	
//    /* Variable to store actual bytes read from flash */
//	  uint32_t actual_size;

//    /* Array to store address */
//    uint8_t addr_array[2] = { 0 };
//		
//	  /* Array to store 256 bytes of config_data taken from flash */
//    uint8_t temp_config_data_array[256] = { 0 };

//    if (config_data_addr != 0)
//    {
//        port_bmi270_itf_end();
//			
//        flash_itf_init();
...
//        spi_flash_release_from_power_down();
//    		spi_flash_read_data(temp_config_data_array, config_data_addr, write_len, &actual_size);    
//			  spi_flash_power_down();
//			
//			  flash_itf_de_init();
//			
...
//        {
//            /* Burst write configuration file data corresponding to user set length */
//            rslt = bmi2_set_regs(BMI2_INIT_DATA_ADDR, (uint8_t *)config_data_addr, write_len, dev);
//        }
//    }
//    else
//    {
                                    ...
                                    struct bmi2_dev *dev)
{
    ...
    if (!remain)
    {
        ...
        {
            rslt = upload_file((dev->config_file_addr + index), index, write_len, dev);
