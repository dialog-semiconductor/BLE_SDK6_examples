 ...
 */
int8_t null_ptr_check(const struct bmi2_dev *dev);
...
int8_t bmi270_init(struct bmi2_dev *dev)
{
    ...
    if (rslt == BMI2_OK)
    {
        ...
        dev->config_size = sizeof(bmi270_config_file);   // NI
        ...
        if (!dev->config_file_addr)
        {
            /* Give the address of the configuration file in flash to
             * the device pointer
             */
            dev->config_file_addr = (uint32_t)bmi270_config_file; // NI
//	            dev->config_file_addr = 0x7000;
        }
