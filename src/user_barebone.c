...
int volatile Adv_interval_ms = INTV_TIME_SHORT_MS;
bool volatile three_second_push = 0;
bool volatile released = 0;
bool volatile sleep = 0;
bool volatile connection = 0;
...
void user_defined_callback(uint8_t status)
{
...
void systick_isr() // systick ISR handler
{
		char* s = "";
		if (sleep)
		{
		...
		uart2_write((uint8_t*)s, strlen(s), user_defined_callback);
...
void negative_GPIO_ISR(void)
{
    ...
    if(sleep && !three_second_push)
    {
   	...
   	else if (!sleep && !three_second_push )
    {
				//remain awake, change advertising interval en connectability
        //GPIO_SetActive(GPIO_LED_PORT, GPIO_LED_PIN);
				s = "\n\rChange advertising";
				if (Adv_interval_ms == INTV_TIME_LONG_MS)
				{
					//change interval to 15ms
					//change advertising mode to CONNECTABLE
					Adv_interval_ms = INTV_TIME_SHORT_MS;
					
				}
				else if (Adv_interval_ms == INTV_TIME_SHORT_MS)
				{
					// change interval to 105ms
					Adv_interval_ms = INTV_TIME_LONG_MS;
				}
...
{ 
	...
	char *s = "\r\nTEST in wakeup_ISR";
	my_data = arch_get_sleep_mode();
	
	if (my_data)
	{
		s = "YES WAKING UP\r\n";
		arch_set_sleep_mode(ARCH_SLEEP_OFF); 				// sleepmode off
	
		periph_init();		//(re)initialise GPIO in and after sleepmode
		
		
		
		SetBits16(CLK_CTRL_REG, SYS_CLK_SEL, 0) ; 	// Select crystal oscillator as clock (precision needed for UART communication)
		three_second_push = 0; 											// reset flag
		systick_start(3000000, 1); 									// start timer generating exeption at 3 seconds
	}
